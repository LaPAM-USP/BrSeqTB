...
⚠ When using the simplified naming convention, the FASTQ filenames do not contain sample number or lane information. In this case, the pipeline will automatically assign arbitrary placeholder values, such as `S1` and `L001`, for downstream steps that require Illumina-style sample/lane identifiers. These placeholders are technical only and should not be interpreted as the original sequencing lane or sample index.
